...
def predict_probs(model, dataloader, device=None, return_ids=True, verbose=True):
    ...
    with torch.no_grad():
        ...
        for batch in iterator:
            ...
            if return_ids:
                # Handles both filename and id field, with fallback
                if "filename" in batch:
                    all_ids.extend(batch["filename"])
                elif "id" in batch:
                    all_ids.extend(batch["id"])
                else:
                    # Optionally: fallback to row index if neither present
                    all_ids.extend([str(i) for i in range(len(probs))])
                    print("⚠️ WARNING: No 'filename' or 'id' in batch; using row indices as IDs.")
