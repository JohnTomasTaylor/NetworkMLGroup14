...
def apply_flooding(loss, b=0.02):
    return (loss - b).abs() + b
...
def train(train_loader, val_loader, model, optimizer, scheduler, fold_idx, device):
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in loop:
            ...
            loss_raw = focal_loss(out, y)  # focal loss
            loss = apply_flooding(loss_raw, b=0.02)  # flooding
            loss.backward()
        ...
        ema_f1 = ema_alpha * ema_f1 + (1 - ema_alpha) * val_f1
...
if __name__ == "__main__":
    ...
    for fold, (train_pid_idx, val_pid_idx) in enumerate(skf.split(patient_ids, patient_labels)):
        ...
        print(f"Original positives: {len(pos_df)}, negatives: {len(neg_df)}"
              f" → After sampling: {balanced_train_df['label'].sum()}")
